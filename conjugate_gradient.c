...
void solve_cg(FILE *fp, int n, double *a, double *b, double *x, double tol);
void solve_pcg(int n, double *a, double *b, double *m_vec);

int main(int argc, char const *argv[]) {
    FILE* fp;
    double *a_vec, *a_tmp, *b, *x;
    int i,n;
    char filename[16];
    ...
    for (i = 1; i < 4; i++) {
        ...
        vector_copy(n,b,x);
        sprintf(filename, "termes_cg_%d.txt",i);
        fp = fopen(filename,"w");
        solve_cg(fp, n, a_vec, b, x, TOL);
        fclose(fp);
        printf("x =\n");
        print_vector(n,x);
...
void solve_cg(FILE *fp, int n, double *a_vec, double *b, double *x, double tol) {
